...
def parse_messier_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    name_en_field = norm_str(row.get("name_en"))
    name_kr_field = norm_str(row.get("name_kr"))
    ...
    # name_en: 필드 우선, 없으면 raw_name에서 추출
    ...
    # name_kr: 필드 우선, 없으면 매핑 시도
    name_kr = name_kr_field or (KO_MAP.get(name_en) if name_en else None)
    ...
    for row in list_data:
        try:
            ...
            out.append({
                ...
                "name_kr": norm_str(row.get("name_kr")),
                "ra": None,
                "dec": None,
...
# ---------------- 메인 ----------------
def main():
    ap = argparse.ArgumentParser(description="messier_catalogue.json + BSC5P(제공 스키마) → 한/영 병기 통합 카탈로그")
